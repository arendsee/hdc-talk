...
#include "/home/z/.morloc/src/morloc/plain/morloclib/cppbase/core.hpp"
...
std::string m24(std::string s0);

double m652(double n2, double n3)
{
    std::cerr << "Entering m" << 652 << std::endl;
    try
    {
        double n5 = morloc_add(n2, n3);
        return(n5);
    } catch (const std::exception& e)
      {
          std::string error_message = "Error in m652 " + std::string(e.what());
...
double m6(double n1)
{
    std::cerr << "Entering m" << 6 << std::endl;
    try
    {
        double n6 = morloc_mul(n1, n1);
        return(n6);
    } catch (const std::exception& e)
      {
          std::string error_message = "Error in m6 " + std::string(e.what());
          std::cerr << error_message << std::endl;
          throw std::runtime_error(error_message);
      }
}

std::vector<double> m7(std::vector<double> n0)
{
    std::cerr << "Entering m" << 7 << std::endl;
    try
    {
        std::vector<double> n7 = morloc_map( std::function<double(double n1)>(m6)
        , n0 );
        return(n7);
    } catch (const std::exception& e)
      {
          std::string error_message = "Error in m7 " + std::string(e.what());
          std::cerr << error_message << std::endl;
          throw std::runtime_error(error_message);
      }
}
...
std::string m24(std::string s0)
{
    std::cerr << "Entering m" << 24 << std::endl;
    try
    {
        std::vector<double> helper0_schema;
        std::vector<double> n4 = deserialize(s0, helper0_schema);
        std::vector<double> n0 = n4;
        double n8 = morloc_fold( std::function<double(double n2,
        double n3)>(m652)
        , 0.0
        , m7(n0) );
        double helper1_schema;
        std::string s9 = serialize(n8, helper1_schema);
        return(s9);
    } catch (const std::exception& e)
      {
          std::string error_message = "Error in m24 " + std::string(e.what());
          std::cerr << error_message << std::endl;
          throw std::runtime_error(error_message);
      }
}
